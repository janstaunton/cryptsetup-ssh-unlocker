        ...
        while True:
            try:
                ...
                await asyncio.wait_for(
                    loop.create_connection(TCPHandshakeProtocol, host, port),
                    timeout=config.getint("connect_timeout", 10),
                )
                ...
                await asyncio.wait_for(
                    self.ssh_unlock(
                        ssh_options, passphrase=config.get("cryptsetup_passphrase"), server_name=config.name
                    ),
                    timeout=config.getint("ssh_connect_timeout", 10),
                )
